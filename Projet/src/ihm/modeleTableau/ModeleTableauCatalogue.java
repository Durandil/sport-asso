...
public class ModeleTableauCatalogue extends AbstractTableModel {
	 ...
	 */
	private final String[] entetes = { "Référence", "Denomination",
			"Quantite en stock", "Prix Initial" };

	/**
	 * Définition d'un constructeur qui retourne un tableau vide avec 1000
	...
	/**
	 * <p>
	 * Ce constructeur permet de créer 3 types de tableaux :
	 * <ul>
	 * <li>le tableau des articles à réapprovisionner dans
	 * {@link FenetreReapprovisionnement}</li>
	 * <li>le catalogue des articles qui sont dans l'état en stock pour le
	 * client dans {@link FenetreCommandeArticle}</li>
	 * <li>le catalogue de l'ensemble des articles en stock et en rupture de
	 * stock que le gérant va pouvoir gérer (ajout,modification,suppression
	 * d'articles) dans {@link FenetreCatalogueGerant}</li>
	 * </ul>
	 * </p>
	 * 
	 * @param pourReapprovisionnement
	 *            Booléen indiquant si le tableau sert à afficher les articles
	 *            ayant besoin d'être réapprovisionnés
	 * 
	 * @param pourTableauGerant
	 *            Booléen indiquant si le catalogue affiché est destiné au
	 *            gérant (true) ou au client (false)
	 */
	public ModeleTableauCatalogue(boolean pourReapprovisionnement,
			boolean pourTableauGerant) {
		super();
		if (pourReapprovisionnement == false) {
			...
			if (pourTableauGerant == true) {
				...
				ArrayList<String> listeDescriptions = SGBD
						.selectListeStringOrdonneCondition("ARTICLE",
								"DESCRIPTION", "IDARTICLE",
								"ETATARTICLE !='Supprimé'");
				ArrayList<Integer> listeStocks = SGBD
						.selectListeIntOrdonneCondition("ARTICLE", "STOCK",
								"IDARTICLE", "ETATARTICLE !='Supprimé'");
				ArrayList<Integer> listeEtats = SGBD
						.selectListeIntOrdonneCondition("ARTICLE",
