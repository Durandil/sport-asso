 ...
 * </ul>
 * Elle est utilisée dans {@link FicheClient} pour afficher les détails d'une
 * commande selectionnée dans un JComboBox
 * </p>
 * 
 * @see FicheClient
 * 
...
public class ModeleTableauStatCommande extends AbstractTableModel {
	 ...
	 */
	private final String[] entetes = { "Référence Article", "Description",
			"Quantité commandée" };

	/**
	 * Construit un tableau avec l'identifiant, la description et la quantité
	 * commandée de chaque article de la commande entrée en paramètre
	 * 
	 * @param idCommande
	 *            Identifiant de la commande dont il faut afficher les
	...
	public ModeleTableauStatCommande(String idCommande) {
		...
		/**
		 * Contient la liste des identifiants des articles de la commande entrée
		 * en paramètre du constructeur
		 */
		ArrayList<String> listeArticles = SGBD
				.selectListeStringOrdonneCondition(
						"LISTING_ARTICLES_COMMANDES", "IDARTICLE", "IDARTICLE",
						"IDCOMMANDE='" + idCommande + "'");
		/**
		 * Contient la liste des quantités commandées associées à chaque article
		 * de la commande
		 */
		ArrayList<String> listeQuantitesCorrespondantes = SGBD
				.selectListeStringOrdonneCondition(
						"LISTING_ARTICLES_COMMANDES", "QUANTITECOMMANDEE",
						"IDARTICLE", "IDCOMMANDE='" + idCommande + "'");
		...
		for (int i = 0; i < listeArticles.size(); i++) {
			...
			donnees[i][1] = SGBD.selectStringConditionString("ARTICLE",
					"DESCRIPTION", "IDARTICLE", listeArticles.get(i));
